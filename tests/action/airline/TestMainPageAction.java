...
public class TestMainPageAction {
	...
	public void testJSONDataIsOK() {
		...
		String routeName = flight.getRoute().getDepartureTerminal().getName() + "-"
				+ flight.getRoute().getArrivalTerminal().getName();
		...
		for (int i = 0; i < mpAc.getData().size(); i++) {
			assertThat(JSON_DATA_NOT_OK_ERROR, lFv.get(i),
					SamePropertyValuesAs.samePropertyValuesAs(mpAc.getData().get(i)));
		}
