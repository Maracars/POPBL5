...
/* BETTER BUT SLOWER VERSION, this takes into account the airport while creating flights */
  CREATE OR REPLACE FUNCTION public.selectdate(
    planeid integer,
    airportid integer)
  RETURNS timestamp without time zone AS
...
select count(*) into weekCount from flight as f join route r on f.route_id = r.id 
	join gate ga on r.arrivalGate_id = ga.id join gate gd on r.departureGate_id = gd.id
	join terminal td on gd.terminal_id = td.id join terminal ta on ga.terminal_id = ta.id
	where ((f.expectedDepartureDate between my_current_time and my_current_time + INTERVAL '7' DAY) and td.airport_id = airportId) 
	or ((f.expectedArrivalDate between my_current_time and my_current_time + INTERVAL '7' DAY) and ta.airport_id = airportId);
	/*borobildute current_time igual hobeto*/
...
if (weekCount < (24 * 7 * maxFlightsPerHour) and (expectedDate < my_current_time + INTERVAL '7' DAY)) then  
	...
	for i in 0..6 LOOP
   	/*HEMEN loop bat jarrikou, eta maximo 7 buelta emongo dau, aurretiik batenbaten maximua baino gutxiau badago
	day horretako orduak begiratuko dittu, 7 buelta emon ostian ez badau topau null bueltauko dau*/
		select count(*) into dayCount from flight as f join route r on f.route_id = r.id 
			join gate ga on r.arrivalGate_id = ga.id join gate gd on r.departureGate_id = gd.id
			join terminal td on gd.terminal_id = td.id join terminal ta on ga.terminal_id = ta.id
			where ((f.expectedDepartureDate between loopDate and loopDate + INTERVAL '1' DAY) and td.airport_id = airportId)
			or ((f.expectedArrivalDate between loopDate and loopDate + INTERVAL '1' DAY) and ta.airport_id = airportId);
      ...
      for j in 0..23 LOOP
		/*HEMEN loop bat jarrikou, eta maximo 24 buelta emongo dau, libre badago ordu horretakua data + minutuak
		bialtzen da*/
			select count(*) into hourCount from flight as f  join route r on f.route_id = r.id 
				join gate ga on r.arrivalGate_id = ga.id join gate gd on r.departureGate_id = gd.id
				join terminal td on gd.terminal_id = td.id join terminal ta on ga.terminal_id = ta.id
				where ((f.expectedDepartureDate between hourLoopDate and hourLoopDate + INTERVAL '1' HOUR) and td.airport_id = airportId)
				or ((f.expectedArrivalDate between hourLoopDate and hourLoopDate + INTERVAL '1' HOUR) and ta.airport_id = airportId);
