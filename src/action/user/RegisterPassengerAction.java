...
public class RegisterPassengerAction extends RegisterAction {
	...
	void userSpecificValidate() {		
		...
		((Passenger) user).setAddress(address);
		((Passenger) user).setName(name);
		((Passenger) user).setSecondName(secondName);		
	...
	public String userSpecificInsert() {
		String ret = SUCCESS;
		ret = HibernateGeneric.saveObject(((Passenger)user).getAddress()) ? SUCCESS : ERROR;
		return ret;
	}
